...
5. Spawn three turtles. Control the first turtle from its starting point to a goal on the map with a 50Hz control that avoid collision with the other turtles (which don't move). Test the algorithm with different configurations/goals.
...
1. A *turtlesim_node* 
2. A *turtle_teleop_key* for controlling the turtle through the keyboard
3. The node *turtle1_controller*, that is used for receiving the custom message containing the command request, applying saturation if necessary and then publishing the correct command to the turtle's *cmd_vel* topic. This node accept the parameters:
   1. *turtlename* to set which turtle to control (default='turtle1')
   2. *sat_v* and *sat_w* to set the saturation limits
4. The node *turtle1_random_cmd*, which generates random command requests for the turtle. This node accepts the parameter *turtlename* to select the turtle for which the commands are generated (and published).
...
float32 ang
```
The *CMakeLists.txt* and the *package.xml* files have been modified accordingly, to allow the usage of this message.

### Code

The code is split into the two scripts *turtle_controller.py* and *turtle_random_cmd.py*.\
The latter repeatedly generates random values for the linear and angular velocity fields of *Vel_custom* and then publish it to the `('%s_custom_cmd' % self.turtlename)` topic.\
The first one generates a subscriber to the topic `('%s_custom_cmd' % self.turtlename)`, where the custom commands are published; this subscriber implements a callback function such that, when the message are received, they are saturated and an equivalent velocity command is published to the `('%s/cmd_vel' % self.turtlename)`, to actually control the turtle.

Since the actual names of the topics are generated from the *turtlename* parameter given when launching the node, the same script can be used to generate multiple node that manage command requests for different turtles.
...
The difference is that this time the script *turtle_random_cmd_limit.py* is launched, instead of *turtle_random_cmd.py*.\
This node take as parameters the values needed to initialize an object of the *Turtle_Control_Publisher* class, described below.
...
When initialized it takes as argument the name of the turtle to be controlled, it's desired (x,y) starting position and the frequency for message publication rate.\
It exploits the *turtlesim*'s service */teleport_absolute* to set the starting position; if the desired starting position is in the lower part of the map it set the turtle at the center of the map.\
A publisher is then created, which publish to the topic for giving custom command to the turtle. Finally, the node is subscribed to the turtle's pose topic, to update the corresponding internal variable of the class.
...
The main method of this class is the class' method *gen_cmd()*. It runs iteratively the subsequent steps, until it finds a couple of values that will keep the turtle in the upper part of the map:

1. Generate a random value of the control inputs
2. Predict the y-axis position that will be reached if that control input is applied
...
def gen_cmd(self):
        ...
        while self.yp < 5.5 :
            ...
            self.yp = self.pose.y - v/w*(cos(self.pose.theta + w*self.t)-cos(self.pose.theta))
```
The control input found in this way is then published to the turtles *[turtlename]_custom_cmd* topic, where they are read and applied by the *turtle_controller* node.\
NOTE: in order to have a correct prediction of the final position the saturation limits should be set greater than the limits used to generate the random control values
```python
if __name__ == '__main__':
    try:
...
A small difference (order of ~10^(-3)) can be noted between the actual value and the predicted value. This is due to the fact that the prediction is done using 1/hz as the time for which the turtle will move with the generated input; this is not the exact real value since some time is spent generating the command and publishing it.
...
```shell
$ roslaunch srd_exercises exercise4_more_turtles.launch
```

### Launchfile

The launchfile does the following:
...
3. Starts a *turtle_controller**i*** node for each turtle; they can be instantiated with different saturation limits, if desired.
4. Starts the *compute_baricenter* node; this node will publish updated information regarding the pose of the baricenter of the formation, every time a turtle update its pose publishing it to the apposite topic. The node also selects which turtle should move (since turtles should move one at a time).
5. Starts *turtle_baricenter_cmd**i*** for each turtle. It detects when the related turtle should start moving and drives it toward the formation baricenter.
...
The procedure followed to control the turtles is as follows:

1. The *compute_baricenter* node keeps a list of the name and the pose of all the turtles in the formation. It also contains a list of subscribers to each turtle's pose topic, so that when a turtle moves the its position and the position of the formation's baricenter are updated (through the callback function).
...
4. Which turtle should move is selected centrally by the *compute_baricenter* node: it publishes to the topic *formation_whose_turn* the name of the turtle that should start moving. This is read by the a subscriber in each turtle's *turtle_baricenter_cmd* node, which checks the message published and moves turtle if the name match.
5. Every time a turtle finish moving, it publishes its name to the *formation_just_moved* topic. This is read by the central node, that can then publish the next turtle's name in the *formation_whose_turn* topic.
...
The launchfile is similar to those used for the other assignements; the only difference is in the node *turtle_apf_cmd*.
...
<launch>
    ...
    <node name="turtle_apf_cmd" pkg="srd_exercises" type="turtle_apf_cmd.py" respawn="false" output="screen" >
      ...
      <param name="k_rep" type="double" value="4.0"/>
    </node>
...
This contain the implementation of the class *APF*, whose method performs all the operations necessary to run the algorithm and to drive the turtle. The class is initialized similarly to the *Baricenter* class, except it contains also the method necessary for publishing the control to drive the moving turtle.
...
The goal to be reached, used for generating the potential field, can be set by writing a Pose message to the *APF_target* topic, to which the node that generates the command is subscribed.\
In this way the field can be changed dynamically during the execution, and the turtle will start moving toward the new goal. For example to set a goal in (5.0,6.0), it can be done by calling
...
The field is generated from the gradients of the attractive and repulsive functions shown in the course (not reported here since GitHub Markdown doesn't support latex-styled math)
...
def repulsive_gradient(self,x,y,x_obs,y_obs):
    # obstacle radius of influence is self.eta_0
    pos = np.array((x,y))
...
The values of k_att and k_rep control the strength of the fields and can be set as parameters when launching the node from the launch file.

To actually move the vehicle along the gradient field, a modified proportional controller has been implemented. The control law is similar to those illustrated for the control of a unicycle in the course "Controllo dei Robot".
...
Finally, the class contains also a method for plotting and saving a figure of the gradient generated by the given goal's and obstacles' positions. This is helpful to check if the gradient is generated properly and to have an instant feedback on the effect given by parameters change.
